...
def time_cuda_kernel(
    t_unit: lp.TranslationUnit, params_np: Mapping[str, np.ndarray[np.dtype[Any]]]
) -> float:
    import pycuda.autoinit  # noqa: F401
    import pycuda.gpuarray as npgpu
    from pycuda.autoinit import context
    from pycuda.compiler import SourceModule

    t_unit = t_unit.copy(target=lp.CudaTarget())

    code = lp.generate_code_v2(t_unit).device_code()
    mod = SourceModule(
        code,
        nvcc="/usr/bin/nvcc",
        options=["-allow-unsupported-compiler", "-Wno-deprecated-gpu-targets",
                 "-O3", "--use_fast_math"],
    )
    knl = mod.get_function("loopy_kernel")
    ...
    gsize, lsize = t_unit.default_entrypoint.get_grid_size_upper_bounds_as_exprs(
        t_unit.callables_table
    )
    gsize = gsize + (1, ) * (3 - len(gsize))
    lsize = lsize + (1, ) * (3 - len(lsize))

    ordered_args = [
        params_cuda[arg.name].gpudata for arg in t_unit.default_entrypoint.args
    ]
    knl.prepare("P" * len(ordered_args))

    # warmup
    for _ in range(10):
        knl.prepared_call(gsize, lsize, *ordered_args)
    ...
    return (t_end - t_start) * 50


def time_pyopencl_kernel(
    t_unit: lp.TranslationUnit,
    cq: cl.CommandQueue,
    params_np: Mapping[str, np.ndarray[np.dtype[Any]]],
) -> float:
...
def get_time_for_idealized_5facemass_knl(cq: cl.CommandQueue | None):
    ...
    params_np = {
        ...
        **{f"out_{i}": np.zeros((162000, 20), np.float64) for i in range(5)},
    }

    transform = fnsm.retrieve(
        fnsm.get_a_matched_einsum(t_unit)[0],
        fnsm.FakeCLDevice("NVIDIA_TITAN_V"),
    ...
    if cq:
        print(
            "[Idealized] Total time ="
            f" {time_pyopencl_kernel(t_unit, cq, params_np):.1f} ms"
        )
    else:
        print(
            f"[Idealized] Total time = {time_cuda_kernel(t_unit, params_np):.1f} ms"
        )


def get_time_for_3dp3_euler_facemass_knl(cq: cl.CommandQueue | None):
    t_unit = get_euler_facemass_kernel()
    ref_t_unit = t_unit
    params_np = {
        **get_params_for_euler_facemass(),
        **{f"out_{i}": np.zeros((162000, 20)) for i in range(5)},
    }
    ...
    1/0
    print(
        f"[Actual] Footprint = {get_footprint_nbytes(ref_t_unit) * 1e-6:.1f} MB"
    )
    if cq:
        print(
            "[Actual] Total time ="
        ...
        print(
            f"[Actual] Total time = {time_cuda_kernel(t_unit, params_np):.1f} ms"
        )
