...
class _MinimalBytesKernelError(RuntimeError):
    pass


class _MinimalFlopsKernelError(RuntimeError):
    pass
...
class _BatchedEinsumKernelWithMinimalBytesGettingActx(BatchedEinsumArrayContext):
    ...
    :class:`_MinimalBytesKernelError` with the translation unit as the
    argument.
    """
    ...
    def transform_loopy_program(self, t_unit):
        ...
        if t_unit.default_entrypoint.tags_of_type(FromArrayContextCompile):
            ...
            raise _MinimalBytesKernelError(t_unit)
        else:
...
class _BatchedEinsumKernelWithMinimalFlopsGettingActx(BatchedEinsumArrayContext):
    ...
    def transform_dag(
        self, dag: "pytato.DictOfNamedArrays"
        ...
        dag = pt.transform.map_and_copy(
            dag, materialize_all_einsums_or_reduces  # type: ignore[assignment]
        )
        ...
        if self.assume_all_indirection_maps_as_non_negative:
            ...
            # type-ignore-reason: mypy is right pytato provides imprecise types.
            dag = pt.transform.map_and_copy(
                dag, tag_indices_as_non_negative  # type: ignore[assignment]
            )
        ...
        def implement_einsum_reduction_inputs_as_substs(expr):
            ...
            if isinstance(expr, pt.Einsum):
                ...
                return pt.Einsum(
                    ...
                    tuple(
                        _make_passthrough_arg(arg, ImplSubstitution())
    ...
    def transform_loopy_program(self, t_unit):
        ...
        if t_unit.default_entrypoint.tags_of_type(FromArrayContextCompile):
            ...
            raise _MinimalFlopsKernelError(t_unit)
        else:
            return super().transform_loopy_program(t_unit)
...
def _get_min_flops_kernel(
    ...
    except _MinimalFlopsKernelError as e:
        (t_unit,) = e.args
...
def _get_min_bytes_kernel(
    ...
    try:
        rhs_clbl(*args, **kwargs)
    except _MinimalBytesKernelError as e:
        (t_unit,) = e.args
        assert isinstance(t_unit, lp.TranslationUnit)
        return t_unit
    else:
...
def get_float64_flops(equation: str, dim: int, degree: int) -> int:
    ...
    op_map = lp.get_op_map(
        t_unit, subgroup_size=1, within=InsnIds(elementwise_insn_ids)
    )
    ...
    c128_ops = {
        op_type: (
            op_map.filter_by(
                dtype=[np.complex128], name=op_type, kernel_name=kernel_name
            ).eval_and_sum({})
        )
        for op_type in ["add", "mul", "div"]
    }
    f64_ops = op_map.filter_by(
        dtype=[np.float64], kernel_name=kernel_name
    ).eval_and_sum({}) + (
        2 * c128_ops["add"] + 6 * c128_ops["mul"] + (6 + 3 + 2) * c128_ops["div"]
    )

    c64_ops = {
        op_type: (
            op_map.filter_by(
                dtype=[np.complex64], name=op_type, kernel_name=kernel_name
            ).eval_and_sum({})
        )
        for op_type in ["add", "mul", "div"]
    }
    f32_ops = op_map.filter_by(
        dtype=[np.float32], kernel_name=kernel_name
    ).eval_and_sum({}) + (
        2 * c64_ops["add"] + 6 * c64_ops["mul"] + (6 + 3 + 2) * c64_ops["div"]
    )
    ...
    for insn_ids in call_kernel_insn_ids:
        (_within_inames,) = {
            t_unit[kernel_name].id_to_insn[insn_id].within_inames
            for insn_id in insn_ids
        }
        (redn_inames,) = {
            t_unit[kernel_name].id_to_insn[insn_id].reduction_inames()
            for insn_id in insn_ids
        }
        if redn_inames:
...
def get_footprint_bytes(equation: str, dim: int, degree: int) -> int:
    ...
    for ary in knl.args:
        if (
            isinstance(ary, ArrayBase)
            and ary.address_space == lp.AddressSpace.GLOBAL
        ):
            nfootprint_bytes += product(ary.shape) * ary.dtype.itemsize
    ...
    for ary in knl.temporary_variables.values():
        if ary.address_space == lp.AddressSpace.GLOBAL:
            ...
            nfootprint_bytes += 2 * product(ary.shape) * ary.dtype.itemsize
